...
this.pontuacaoAtual = this.pontuacaoInicial;
  ...
  create() {
    ...
    // GHOSTFACE
    this.ghostface = this.physics.add
    ...
    //ESQUELETO 1
    this.mob1 = this.physics.add.image(210, 100, 'esqueletocity')
      .setScale(1.5)
      .setSize(40,90)
      .setOffset(30,20);
    //ESQUELETO 2
    this.mob2 = this.physics.add.image(1510, 200, 'esqueletocity')
      .setScale(1.5)
      .setSize(40,90)
      .setOffset(30,20);
    //ESQUELETO 3
    this.mob3 = this.physics.add.image(3010, 200, 'esqueletocity')
      .setScale(1.5)
      .setSize(40,90)
      .setOffset(30,20);
    //ESQUELETO 4
    this.mob4 = this.physics.add.image(4510, 200, 'esqueletocity')
      .setScale(1.5)
      .setSize(40,90)
      .setOffset(30,20);
    ...
    //Esqueletos chão
    //ESQUELETO 5
    this.mob5 = this.physics.add.image(820, 400, 'mob1city')
      .setScale(1.8)
      .setSize(40,90)
      .setOffset(30,20);
    //ESQUELETO 6
    this.mob6 = this.physics.add.image(1910, 400, 'esqueletocity')
      .setScale(1)
      .setSize(40,90)
      .setOffset(30,20);
    this.mob61 = this.physics.add.image(2110, 400, 'esqueletocity')
      .setScale(1)
      .setSize(40,90)
      .setOffset(30,20);
    this.mob62 = this.physics.add.image(2310, 400, 'esqueletocity')
      .setScale(1)
      .setSize(40,90)
      .setOffset(30,20);
    
    //ESQUELETO 7
    this.mob7 = this.physics.add.image(3345, 400, 'esqueletocity')
      .setScale(1.8)
      .setSize(40,90)
      .setOffset(30,20);
    ...
    this.physics.add.overlap(this.bullets, this.ghostface, this.bulletHitEnemy, null, this);
    ...
    this.physics.add.overlap(this.personagem, this.ghostface, this.mobHitpersonagem, null, this);
    ...
    this.anims.create({
      key: 'andarGhost',
      frames: this.anims
        .generateFrameNumbers('rambo', { start: 2, end: 3 }),
      frameRate: 10,
      repeat: -1
    });
   ...
   this.anims.create({
    ...
    this.rKey.on('up', () => {
      ...
      this.physics.add.collider(this.bullets, this.ghostface, this.bulletHitEnemy, null, this);
      ...
      this.physics.add.collider(this.personagem, this.ghostface, this.mobHitpersonagem, null, this);
    });
  ...
  update() {
    ...
    if (this.controles.left.isDown) {
      ...
      if (this.controles.right.isDown) {
        ...
        if (this.controles.down.isDown) {
          ...
          if (this.controles.space.isDown && this.bullets.bulletQuantity > 0) {
    ...
    if (this.personagem.x >= this.cidade.displayWidth - 100 && this.vidaGhostface == 0) {
    ...
    //ghostFace
    this.barraVidaGhostface.x = this.ghostface.x - 10;
    this.barraVidaGhostface.y = this.ghostface.y - 75;
    this.barraVidaGhostface.width = this.vidaGhostface;
  ...
  bulletHitEnemy(enemy, bullet) {
    ...
    if (enemy === this.mob1 && this.vidamob1 > 0) {
      this.vidamob1 -= 20;
      bullet.destroy();
    } else if (enemy === this.ghostface && this.vidaGhostface > 0) {
      this.vidaGhostface -= 10;
      bullet.destroy();
    ...
    if (enemy === this.ghostface && this.vidaGhostface <= 10) {
      this.ghostface.anims.play('ghostMorrendo');
      bullet.destroy();
      if(this.vidaGhostface <= 0){
      this.ghostface.destroy();
...
verificarColisoes() {
  ...
  ghostShoot() {
    ...
    if (this.vidaGhostface > 0) {
      ...
      const shootX = this.ghostface.x - 50; // Ajuste conforme necessário
      const shootY = this.ghostface.y; // Ajuste conforme necessário
      // Faz o Ghost atirar
      if (this.personagem.x < this.ghostface.x) {
        this.ghostface.flipX = true;
        this.ghostface.anims.play('ghostAtacando');
        this.bullets2.fireBullet(shootX, shootY, 'left'); // Ajuste a direção do tiro conforme necessário
      }
      else {
        this.ghostface.flipX = false;
        this.ghostface.anims.play('ghostAtacando');
  ...
  ghostWalk() {
    //Move ghostFace
    if(this.vidaGhostface > 0){
      if (this.ghostface.x <= 5000) {
        this.ghostface.setVelocityX(200); // Move para a direita
        this.ghostface.flipX = false;
        this.ghostface.anims.play('andarGhost');
      } else if (this.ghostface.x >= 5400) {
        this.ghostface.setVelocityX(-200); // Move para a esquerda
        this.ghostface.flipX = true;
        this.ghostface.anims.play('andarGhost');
      }
    }
